...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (pg_num_rows($check_result) == 0) {
        echo json_encode(['success' => false, 'message' => 'L\'esame non esiste!']);
        pg_free_result($check_result);
        pg_close($conn);
        exit;
    }else{

        $query = "DELETE FROM esami WHERE codice = $1";
        $result = pg_query_params($conn, $query, array($codice));
        if (!$result) {
            echo json_encode(['success' => false, 'message' => 'Errore durante l\'eliminazione dell\'esame: ' . pg_last_error($conn)]);
            exit;
        }
